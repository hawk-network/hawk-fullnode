...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   create_accounts({ N(hawknwk.token), N(hawknwk.ram), N(hawknwk.ramfee), N(hawknwk.stake),
      N(hawknwk.bpay), N(hawknwk.vpay), N(hawknwk.saving), N(hawknwk.names) });
   ...
   auto act = mutable_variant_object()
         ("issuer",       "hawknwk")
         ("maximum_supply", hawknwk::chain::asset::from_string("1000000000.0000 SYS"));
   push_action(N(hawknwk.token), N(create), N(hawknwk.token), act );

   // issue
   for (account_name a: accs) {
      push_action( N(hawknwk.token), N(issue), "hawknwk", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", hawknwk::chain::asset::from_string("999.0000 SYS") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   // iterate over scope
   hawknwk::chain_apis::read_only plugin(*(this->control), fc::microseconds::maximum());
   hawknwk::chain_apis::read_only::get_table_by_scope_params param{N(hawknwk.token), N(accounts), "inita", "", 10};
   hawknwk::chain_apis::read_only::get_table_by_scope_result result = plugin.read_only::get_table_by_scope(param);

   BOOST_REQUIRE_EQUAL(4u, result.rows.size());
   BOOST_REQUIRE_EQUAL("", result.more);
   if (result.rows.size() >= 4) {
      BOOST_REQUIRE_EQUAL(name(N(hawknwk.token)), result.rows[0].code);
      BOOST_REQUIRE_EQUAL(name(N(inita)), result.rows[0].scope);
      BOOST_REQUIRE_EQUAL(name(N(accounts)), result.rows[0].table);
      BOOST_REQUIRE_EQUAL(name(N(hawknwk)), result.rows[0].payer);
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   for (account_name a: accs) {
      push_action( N(hawknwk.token), N(issue), "hawknwk", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", hawknwk::chain::asset::from_string("10000.0000 SYS") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "hawknwk")
         ("maximum_supply", hawknwk::chain::asset::from_string("1000000000.0000 AAA"));
   push_action(N(hawknwk.token), N(create), N(hawknwk.token), act );
   // issue
   for (account_name a: accs) {
      push_action( N(hawknwk.token), N(issue), "hawknwk", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", hawknwk::chain::asset::from_string("9999.0000 AAA") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "hawknwk")
         ("maximum_supply", hawknwk::chain::asset::from_string("1000000000.0000 CCC"));
   push_action(N(hawknwk.token), N(create), N(hawknwk.token), act );
   // issue
   for (account_name a: accs) {
      push_action( N(hawknwk.token), N(issue), "hawknwk", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", hawknwk::chain::asset::from_string("7777.0000 CCC") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "hawknwk")
         ("maximum_supply", hawknwk::chain::asset::from_string("1000000000.0000 BBB"));
   push_action(N(hawknwk.token), N(create), N(hawknwk.token), act );
   // issue
   for (account_name a: accs) {
      push_action( N(hawknwk.token), N(issue), "hawknwk", mutable_variant_object()
                  ("to",      name(a) )
                  ("quantity", hawknwk::chain::asset::from_string("8888.0000 BBB") )
                  ("memo", "")
                  );
   }
   produce_blocks(1);
   ...
   hawknwk::chain_apis::read_only plugin(*(this->control), fc::microseconds::maximum());
   hawknwk::chain_apis::read_only::get_table_rows_params p;
   p.code = N(hawknwk.token);
   p.scope = "inita";
   p.table = N(accounts);
   p.json = true;
   p.index_position = "primary";
   hawknwk::chain_apis::read_only::get_table_rows_result result = plugin.read_only::get_table_rows(p);
   BOOST_REQUIRE_EQUAL(4u, result.rows.size());
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("hawknwk", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("hawknwk", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("hawknwk", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("hawknwk", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_FIXTURE_TEST_CASE( get_table_by_seckey_test, TESTER ) try {
   ...
   auto bidname = [this]( const account_name& bidder, const account_name& newname, const asset& bid ) {
      return push_action( N(hawknwk), N(bidname), bidder, fc::mutable_variant_object()
                          ("bidder",  bidder)
                          ("newname", newname)
                          ("bid", bid)
                          );
   ...
   bidname(N(inita), N(com), hawknwk::chain::asset::from_string("10.0000 SYS"));
   bidname(N(initb), N(org), hawknwk::chain::asset::from_string("11.0000 SYS"));
   bidname(N(initc), N(io), hawknwk::chain::asset::from_string("12.0000 SYS"));
   bidname(N(initd), N(html), hawknwk::chain::asset::from_string("14.0000 SYS"));
   produce_blocks(1);

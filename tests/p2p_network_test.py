...
# hawknwk should have the same key as defproducera
hawknwk = copy.copy(defproduceraAccount)
hawknwk.name = "hawknwk"
...
wasmFile="hawknwk.system.wasm"
abiFile="hawknwk.system.abi"
Print("\nPush system contract %s %s" % (wasmFile, abiFile))
trans=node0.publishContract(hawknwk.name, wasmFile, abiFile, waitForTransBlock=True)
if trans is None:
    Utils.errorExit("Failed to publish hawknwk.system.")
else:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, hawknwk)

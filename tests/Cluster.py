...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="hawknwk-ignition-wd/bootlog.txt"
    __configDir="etc/hawknwk/"
    __dataDir="var/lib/"
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
        ...
        self.hawknwkAccount=self.defProducerAccounts["hawknwk"]= Account("hawknwk")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
        ...
        specificExtraNodeosArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodeos flags on launcher), example: { "5" : "--plugin hawknwk::test_control_api_plugin" }
        """
        ...
        if not self.walletd:
            nodeosArgs += " --plugin hawknwk::wallet_api_plugin"
        if self.enableMongo:
            nodeosArgs += " --plugin hawknwk::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        ...
        self.hawknwkAccount=self.defProducerAccounts["hawknwk"]
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.hawknwkAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse hawknwk private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(totalNodes, biosHost, biosPort, walletMgr, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by hawknwk-launcher."""
        ...
        hawknwkName="hawknwk"
        hawknwkKeys=producerKeys[hawknwkName]
        hawknwkAccount=Account(hawknwkName)
        hawknwkAccount.ownerPrivateKey=hawknwkKeys["private"]
        hawknwkAccount.ownerPublicKey=hawknwkKeys["public"]
        hawknwkAccount.activePrivateKey=hawknwkKeys["private"]
        hawknwkAccount.activePublicKey=hawknwkKeys["public"]
        producerKeys.pop(hawknwkName)

        if not walletMgr.importKey(hawknwkAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (hawknwkName))
            return None

        initialFunds="1000000.0000 {0}".format(CORE_SYMBOL)
        Utils.Print("Transfer initial fund %s to individual accounts." % (initialFunds))
        trans=None
        contract="hawknwk.token"
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"hawknwk\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init hawknwk transfer")
            opts="--permission hawknwk@active"
            if name != "hawknwk":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from hawknwk.token to %s." % (name))
                    return None
    ...
    def bootstrap(totalNodes, prodCount, totalProducers, biosHost, biosPort, walletMgr, onlyBios=False):
        ...
        contract="hawknwk.bios"
        contractDir="unittests/contracts/%s" % (contract)
        wasmFile="%s.wasm" % (contract)
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(hawknwkAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, hawknwkAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission hawknwk@active"
                    myTrans=biosNode.pushMessage("hawknwk", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("hawknwk", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but hawknwk).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "hawknwk"
            ret=Utils.waitForBool(lam)
            if not ret:
                Utils.Print("ERROR: Block production handover failed.")
                return None

        hawknwkTokenAccount=copy.deepcopy(hawknwkAccount)
        hawknwkTokenAccount.name="hawknwk.token"
        trans=biosNode.createAccount(hawknwkTokenAccount, hawknwkAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (hawknwkTokenAccount.name))
            return None

        hawknwkRamAccount=copy.deepcopy(hawknwkAccount)
        hawknwkRamAccount.name="hawknwk.ram"
        trans=biosNode.createAccount(hawknwkRamAccount, hawknwkAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (hawknwkRamAccount.name))
            return None

        hawknwkRamfeeAccount=copy.deepcopy(hawknwkAccount)
        hawknwkRamfeeAccount.name="hawknwk.ramfee"
        trans=biosNode.createAccount(hawknwkRamfeeAccount, hawknwkAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (hawknwkRamfeeAccount.name))
            return None

        hawknwkStakeAccount=copy.deepcopy(hawknwkAccount)
        hawknwkStakeAccount.name="hawknwk.stake"
        trans=biosNode.createAccount(hawknwkStakeAccount, hawknwkAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (hawknwkStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(hawknwkTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=hawknwkTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (hawknwkTokenAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to hawknwk contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (hawknwkAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to hawknwk contract.")
            return None
        ...
        Utils.Print("Verify hawknwk issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountEosBalanceStr(hawknwkAccount.name)
        if expectedAmount != actualAmount:
            Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                        (expectedAmount, actualAmount))
            return None

        contract="hawknwk.system"
        contractDir="unittests/contracts/%s" % (contract)
        wasmFile="%s.wasm" % (contract)
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (hawknwkAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (hawknwkAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (hawknwkTokenAccount.name, name))
                return None
        ...
        opts="--permission %s@active" % (hawknwkAccount.name)
        trans=biosNode.pushMessage(hawknwkAccount.name, action, data, opts)
        Utils.Print("Cluster bootstrap done.")

...
public:
   void deploy_contract( bool call_init = true ) {
      set_code( config::system_account_name, contracts::hawknwk_system_wasm() );
      set_abi( config::system_account_name, contracts::hawknwk_system_abi().data() );
      if( call_init ) {
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "hawknwk_global_state", data, abi_serializer_max_time );
   }
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(hawknwk.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create hawknwk.msig and hawknwk.token
        create_accounts({N(hawknwk.msig), N(hawknwk.token), N(hawknwk.ram), N(hawknwk.ramfee), N(hawknwk.stake), N(hawknwk.vpay), N(hawknwk.bpay), N(hawknwk.saving) });
        // Set code for the following accounts:
        //  - hawknwk (code: hawknwk.bios) (already set by tester constructor)
        //  - hawknwk.msig (code: hawknwk.msig)
        //  - hawknwk.token (code: hawknwk.token)
        // set_code_abi(N(hawknwk.msig), contracts::hawknwk_msig_wasm(), contracts::hawknwk_msig_abi().data());//, &hawknwk_active_pk);
        // set_code_abi(N(hawknwk.token), contracts::hawknwk_token_wasm(), contracts::hawknwk_token_abi().data()); //, &hawknwk_active_pk);

        set_code_abi(N(hawknwk.msig),
                     contracts::hawknwk_msig_wasm(),
                     contracts::hawknwk_msig_abi().data());//, &hawknwk_active_pk);
        set_code_abi(N(hawknwk.token),
                     contracts::hawknwk_token_wasm(),
                     contracts::hawknwk_token_abi().data()); //, &hawknwk_active_pk);

        // Set privileged for hawknwk.msig and hawknwk.token
        set_privileged(N(hawknwk.msig));
        set_privileged(N(hawknwk.token));

        // Verify hawknwk.msig and hawknwk.token is privileged
        const auto& hawknwk_msig_acc = get<account_object, by_name>(N(hawknwk.msig));
        BOOST_TEST(hawknwk_msig_acc.privileged == true);
        const auto& hawknwk_token_acc = get<account_object, by_name>(N(hawknwk.token));
        BOOST_TEST(hawknwk_token_acc.privileged == true);


        // Create SYS tokens in hawknwk.token, set its manager as hawknwk
        ...
        create_currency(N(hawknwk.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to hawknwk.system
        issue(N(hawknwk.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(hawknwk.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "hawknwk");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), hawknwk_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), hawknwk_assert_message_exception);

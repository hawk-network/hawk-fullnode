...
CONTRACT test_ram_limit : public contract {
   ...
   using contract::contract;
      ...
      ACTION setentry( name payer, uint64_t from, uint64_t to, uint64_t size ) {
         ...
         hawknwk::print("test_ram_limit::setentry ", hawknwk::name{self}, "\n");
      ...
      ACTION rmentry( uint64_t from, uint64_t to ) {
         const auto self = get_self();
         hawknwk::print("test_ram_limit::rmentry ", hawknwk::name{self}, "\n");
         test_table table( self, self.value );
         for ( int key = from; key <=to; ++key ) {
            auto itr = table.find(key);
            hawknwk_assert ( itr != table.end(), "could not find test_table entry" );
            table.erase(itr);
         }
      }
      ...
      ACTION printentry( uint64_t from, uint64_t to ) {
         const auto self = get_self();
         hawknwk::print("test_ram_limit::printout ", hawknwk::name{self}, ":");
         test_table table( self, self.value );
         for ( int key = from; key <= to; ++key ) {
            auto itr = table.find(key);
            hawknwk::print("\nkey=", key);
            hawknwk_assert ( itr != table.end(), "could not find test_table entry" );
            hawknwk::print(" size=", itr->data.size());
         }
      }
   ...
   private:
      ...
      typedef hawknwk::multi_index< "test.table"_n, test> test_table;
};

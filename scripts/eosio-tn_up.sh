...
if [ "$PWD" != "$HAWK-NETWORK_HOME" ]; then
    echo $0 must only be run from $HAWK-NETWORK_HOME
    exit -1
fi
...
datadir=var/lib/node_$HAWK-NETWORK_NODE
...
if [ -z "$HAWK-NETWORK_LEVEL" ]; then
    echo starting with no modifiers
    relaunch
    if [ "$connected" -eq 0 ]; then
        HAWK-NETWORK_LEVEL=replay
    else
        exit 0
    fi
fi

if [ "$HAWK-NETWORK_LEVEL" == replay ]; then
    echo starting with replay
    relaunch --hard-replay-blockchain
    if [  "$connected" -eq 0 ]; then
        HAWK-NETWORK_LEVEL=resync
    else
        exit 0
    fi
fi
if [ "$HAWK-NETWORK_LEVEL" == resync ]; then
